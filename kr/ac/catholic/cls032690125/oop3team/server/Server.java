...
public class Server {
    ...
    public Server(ProgramProperties control) throws ServerIgnitionFailureException {
        try{
            ...
            this.userProfileDAO = new UserProfileDAO(this);
    ...
    public void dispatch(ServerClientHandler handler, ClientOrderBasePacket order) {
        ...
        try {
            if (order instanceof CUpdateUserProfileRequest request){
                String userId = request.getUserId();
                String displayName = request.getDisplayName();
                String status = request.getStatusMessage();

                boolean success = false;
                String message = "error";

                try {
                    userProfileDAO.updateUserProfile(userId,displayName,status);
                    success = true;
                    message = "success";
                }catch (Exception e){
                    message = "error"+e.getMessage();
                }
                handler.send(new SUpdateUserProfileResponse(request.getRequestId(),success,message));
            }
        }catch (Exception e){
